...
import type { CustomScriptHookConfig } from "@blud/runtime-scripting";
    ...
    <div className="space-y-3 px-1 pb-1">
      ...
      {hooks.map((hook) => {
        ...
        return (
          ...
          >
            {definition?.readOnly || hook.type === "custom_script" ? (
              <CustomScriptReadonlyInspector hook={hook} />
            ) : definition ? (
              <>
...
function CustomScriptReadonlyInspector({ hook }: { hook: SceneHook }) {
  ...
  return (
    <div className="space-y-3">
      <div className={cn(ROW_SURFACE_CLASS, "space-y-2 p-3")}>
        <div className="text-[10px] font-medium tracking-[0.18em] text-foreground/42 uppercase">Generated Script</div>
        <div className="text-xs text-foreground/72">
          This hook was generated by the HTML/JS importer and is read-only in the editor.
        </div>
        {typeof config.runtime === "string" ? (
          <div className="text-[11px] text-foreground/52">Runtime: <span className="font-mono text-foreground/72">{config.runtime}</span></div>
        ) : null}
      </div>

      {originEntries.length > 0 ? (
        <FieldGroup label="Origin">
          <div className={cn(ROW_SURFACE_CLASS, "space-y-1.5 p-2.5")}>
            {originEntries.map(([key, value]) => (
              <div className="flex items-start justify-between gap-3 text-xs" key={key}>
                <span className="text-foreground/46">{startCase(key)}</span>
                <span className="max-w-[16rem] break-all text-right font-mono text-foreground/72">
                  {formatGameplayValue(value)}
                </span>
              </div>
            ))}
          </div>
      ...
      <FieldGroup label="Capabilities">
        <div className="flex flex-wrap gap-1.5">
          ...
          )) : <div className="text-xs text-foreground/44">No explicit capabilities requested.</div>}
        </div>
      </FieldGroup>

      <FieldGroup label={`Diagnostics (${diagnostics.length})`}>
        <div className="space-y-2">
          {diagnostics.length > 0 ? diagnostics.map((diagnostic, index) => {
            ...
            const message = typeof diagnostic.message === "string" ? diagnostic.message : "Generated import diagnostic.";
            const file = typeof diagnostic.file === "string" ? diagnostic.file : "";

            return (
              <div className={cn(ROW_SURFACE_CLASS, "space-y-1.5 p-2.5")} key={`${hook.id}:diagnostic:${index}`}>
                <div className="flex items-center justify-between gap-3">
                  <span className="text-[10px] font-medium tracking-[0.16em] text-foreground/42 uppercase">{severity}</span>
                  {file ? <span className="max-w-[14rem] truncate text-[10px] font-mono text-foreground/42">{file}</span> : null}
                </div>
                <div className="text-xs leading-5 text-foreground/72">{message}</div>
              </div>
            );
          }) : <div className="text-xs text-foreground/44">No diagnostics were attached to this generated script.</div>}
        </div>
      </FieldGroup>

      <FieldGroup label="Source Preview">
        <pre className={cn(ROW_SURFACE_CLASS, "max-h-72 overflow-auto p-3 text-[11px] leading-5 text-foreground/74")}>
          <code>{source || "// No generated source available."}</code>
        </pre>
      </FieldGroup>
    </div>
  );
